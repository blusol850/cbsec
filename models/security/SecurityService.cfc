...
component implements="ISecurityService" singleton{
	...
	// Dependencies
	property name="authorService" 		inject="id:authorService@cbsec";
	...
	*/
	boolean function userValidator( required struct rule, any controller ){
		...
		if( author.getisloaded() AND author.getisLoggedIn() ){

			// Check if the rule requires roles TODO:
			// if( len( rule.roles ) ){
	...
	*/
	function getAuthorSession(){
		...
		if( authorID ){
			...
			var author = wirebox.getInstance( "Author@cbsec" )
				.where( "authorID", authorID )
				// .andWhere( "isActive", true ) TODO:
				.get();

			// If user found?
			if( arrayLen( author.$renderData() ) ){
				// author.setLoggedIn( true ); TODO:
				return author.get()[1];
			}
		}
	...
	*/
	struct function authenticate( required username, required password ){
		...
		// 	isDeleted 	= false
		// } );
		var oAuthor = wirebox.getInstance( "Author@cbsec" )
				.where( "username", arguments.username )
				// .andWhere( "isActive", true ) TODO:
				// .andWhere( "isDeleted", false ) TODO:
				.get().toArray();
		...
		for ( author in oAuthor){
			// Determine password type
			var isBcrypt = ( findNoCase( "$", author.getPassword() ) ? true : false );
			// Hash password according to algorithm
			var isSamePassword = false;
			if( isBcrypt ){
				try{
					isSamePassword = variables.bCrypt.checkPassword( arguments.password, author.getPassword() );
				...
				// Legacy hash compare
				isSamePassword = ( compareNoCase( hash( arguments.password, "SHA-256" ), author.getPassword() ) eq 0 ? true : false );
			}
			...
			if( isSamePassword ){
				...
				if( !isBcrypt ){
					author.setPassword( encryptString( arguments.password ) );
				}
				// Set last login date
				// updateAuthorLoginTimestamp( author ); TODO:
				
				// User authenticated, mark and return
				results.isAuthenticated = true;
				results.author = oAuthor[1];
			}
			break;
		}
